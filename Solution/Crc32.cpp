...
//Edit :D
void Crc32::findReverseAscii_bruteforce(UInt32 desiredCrc, UInt8* result, int level, int max_level)
{
	for (int i = 0; i < 93; i++) {
		result[level] = i;
		
		if(level == max_level)
			findReverseAscii_check(desiredCrc, result, max_level);
		else
			findReverseAscii_bruteforce(desiredCrc, result, level + 1, max_level);
	}
}

void Crc32::findReverseAscii_check(UInt32 desiredCrc, UInt8* result, int max_level)
{
    Crc32 crc32;
	char  output[max_level + 5];
	
	crc32.set(crc_);
	
	for (int i = 0; i <= max_level; i++)
		crc32.append(TO_CHAR2(result[i]));
	
	const UInt8* reverse = crc32.findReverse(desiredCrc);

	// If the reverse CRC has valid ASCII characters, we found a solution.           
	if (IS_CHAR2(reverse[0]) && IS_CHAR2(reverse[1]) && 
		IS_CHAR2(reverse[2]) && IS_CHAR2(reverse[3])) {
		
		for (int i = 0; i <= max_level; i++)
			output[i] = (char)TO_CHAR2(result[i]);
		
		output[max_level + 1] = (char)reverse[0];
		output[max_level + 2] = (char)reverse[1];
		output[max_level + 3] = (char)reverse[2];
		output[max_level + 4] = (char)reverse[3];
		output[max_level + 5] = 0;
